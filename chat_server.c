...
void *client_session(void *args)
{
   ...
   for(int i = 0; i < all_users_size; ++i) // check for duplicate usernames
   {
   ...
   while(read(client_sd,request,sizeof(request)) > 0)
   {
      ...
      if(current_user.in_room > 0)
      {
         ...
         record_custom_chat(chat_log, current_user);
         memset(buffer,0,sizeof(buffer));
      ...
      for (int i = 0; i < total_rooms; i++)
      {
         if(strcmp(arr_chat_room[i].room_name,name) == 0)
         {
            for(int j = 0; j < arr_chat_room[i].users_in_room_size; j++)
            {
               ...
               {
                  write(user.user_sd, response, sprintf(response, "ERROR: You already entered this room\n"));
                  pthread_mutex_unlock(lock_unlock);
...
{
	FILE *log_file = fopen("default_.log", "a");
...
void record_custom_chat(char *message, chat_user user)
{
   ...
   for(int i = 0; i < total_rooms; i++)
   {
      ...
      {    
         if (strcmp(arr_chat_room[i].users_in_room[j].username, user.username) == 0)
         {
            snprintf(buf,sizeof(buf),"%s%s",arr_chat_room[i].room_name,"_.log");
            FILE *log_file = fopen(buf, "a");
            fprintf(log_file,"<%s log> %s",arr_chat_room[i].room_name,message);
            fclose(log_file);
         }                     
      }                  
   }  
   ...
   for(int i = 0; i < total_rooms; i++)
   {
      ...
      {    
         if (strcmp(arr_chat_room[i].users_in_room[j].username, user.username) == 0)
         {
            ...
            while((read_file = getline(&line, &len, log_file)) != -1)
            {
               write(user.user_sd, response, sprintf(response, "%s",line));
            }
